            ...
            <DialogContent className="max-w-3xl w-[95vw] md:w-full overflow-hidden p-0 border-none shadow-2xl rounded-2xl md:rounded-[2rem]">
                ...
                <div className="bg-gradient h-32 md:h-40 w-full relative">
                    <div className="absolute -bottom-14 left-4 md:left-8 flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-6 text-center md:text-left w-full md:w-auto px-4 md:px-0">
                        <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl md:rounded-3xl bg-white shadow-2xl flex items-center justify-center text-primary font-bold text-3xl md:text-4xl border-4 border-white">
                            {(agent.firstName?.[0] || '') + (agent.lastName?.[0] || '')}
                        </div>
                        <div className="md:mb-4">
                            <p className="text-2xl md:text-3xl font-bold text-white md:text-white drop-shadow-none md:drop-shadow-md bg-white md:bg-transparent px-4 py-1 rounded-xl md:p-0 shadow-sm md:shadow-none">{fullName}</p>
                            <div className="flex flex-col md:flex-row items-center gap-2 mt-1">
                                <Badge className={agent.role === 'AGENT' ? 'bg-gradient text-white border-none px-3 py-1 text-[10px]' : 'bg-yellow-101 text-yellow-700 border-none px-3 py-1 text-[10px]'}>
                                    {agent.role === 'AGENT' ? 'Approved Agent' : 'Pending Approval'}
                                </Badge>
                                <span className="text-gray-500 md:text-white text-[10px] md:text-xs font-medium opacity-80 uppercase tracking-widest">ID: {agent.id}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 md:p-10 pt-28 md:pt-20 space-y-8 md:space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
                        ...
                        <div className="space-y-6 md:space-y-8">
                            <div>
                                <h4 className="text-[10px] font-bold text-gradient uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                        ...
                        <div className="space-y-6 md:space-y-8">
                            <div>
                                ...
                                <div className="space-y-4 bg-green-50/30 p-5 rounded-2xl md:rounded-3xl border border-green-100/50">
                                    ...
                                    <div className="flex justify-between items-center text-xs">
                                        ...
                                        <span className={`font-bold ${agent.role === 'AGENT' ? 'text-gradient' : 'text-yellow-600'}`}>
                            ...
                            <div className="p-5 md:p-6 bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl md:rounded-3xl text-white shadow-xl relative overflow-hidden group">
                                ...
                                <div className="relative z-10 flex items-center gap-4">
                                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient flex items-center justify-center shadow-lg flex-shrink-0">
                                        <Briefcase className="w-5 h-5 md:w-6 md:h-6 text-white" />
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-bold text-gradient/80 uppercase tracking-widest">Portal Access</p>
                ...
                <DialogFooter className="p-6 md:p-8 border-t bg-gray-50/50 rounded-b-2xl md:rounded-b-[2rem] flex flex-col-reverse sm:flex-row gap-3">
                    <Button variant="outline" onClick={onClose} className="w-full sm:w-auto h-12 px-8 rounded-xl md:rounded-2xl border-gray-200 hover:bg-white hover:text-gradient transition-all font-bold">
                        Close Profile
                    </Button>
...
const InfoRow = ({ icon: Icon, label, value }: { icon: LucideIcon, label: string, value: string }) => (
    <div className="flex items-center gap-4 group">
        <div className="w-10 h-10 rounded-2xl bg-white border border-gray-100 shadow-sm flex items-center justify-center text-gray-400 group-hover:text-gradient group-hover:border-gradient transition-all">
